export function getAppointmentSetterSkill() {
  const timezoneOffset = parseInt(process.env.TIMEZONE_OFFSET || '-3');
  // Ajustamos la fecha actual al offset local para el prompt
  const now = new Date();
  const localDate = new Date(now.getTime() + timezoneOffset * 60 * 60 * 1000);
  
  const dateString = localDate.toLocaleDateString('es-ES', { 
    weekday: 'long', 
    year: 'numeric', 
    month: 'long', 
    day: 'numeric' 
...
3. **Confirmación**: Antes de agendar, confirma con el usuario la fecha y hora exacta. **Verifica siempre que el día de la semana coincida con la fecha (ej. si hoy es jueves 23, mañana es viernes 24)**.
4. **Agendamiento**: Una vez confirmado, utiliza \`scheduleAppointmentTool\` para crear el evento.
5. **Contexto Temporal**: Hoy es ${dateString}. 
